...
Metrics are calculated server-side for the executed dataset only. If `datasetId` is omitted, Core executes the first configured dataset and returns only metrics attached to that dataset; if `datasetId` is provided, Core returns only metrics attached to the requested dataset. Core does not execute other datasets to satisfy metrics. The execution order is: dataset static filters, required panel filters, optional panel filters selected by `metric.filterIds`, search, dataset transformation, `metric.conditions`, aggregation, then pagination for visible rows only. For `LATEST_BY_RELATION`, metric conditions run after grouping, so a metric counting status E1 sees the latest grouped record, not historical rows that were superseded. Response pagination only affects `datasets[].rows`, never metric values. The configuration never accepts SQL, JavaScript, expressions, or field names as logic; all references use ids.
