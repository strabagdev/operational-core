...
`organizationId` is not included in the token. Authorization re-reads the `ExternalApp` and derives the effective organization from `ExternalApp.organizationId` on every protected request.
...
- verifies `User.active`, `ExternalApp.active`, and current membership in the application's organization;
...
| 401 | `REFRESH_APP_INVALID` | App was deleted or the user no longer belongs to the application's organization. |
...
| 401 | `TOKEN_APP_INVALID` | Token app was deleted, no longer matches its `clientId`, or the user no longer belongs to the application's organization. |
| 403 | `TOKEN_APP_INACTIVE` | Token app is inactive. |
| 500 | `API_AUTH_SECRET_MISSING` | `API_AUTH_SECRET` is not configured at runtime. |

## GET /api/v1/context

Returns the authenticated user's operational context for the organization selected by the authenticated external application and the active contracts available inside that organization.
...
`organization` is `ExternalApp.organization`, validated through the authenticated user's membership in that organization. `contracts` contains only active contracts from that organization. The `role` value is the effective membership role for that organization and is currently either `ADMIN` or `MEMBER`.
...
Reusable API helpers authenticate the Bearer token, load the real user, validate the token app still exists and is active, load the requested active contract, verify that the contract belongs to the authenticated app organization, and verify server-side that the user has a membership in that organization.

Clients cannot provide `organizationId` or `role`; both are derived server-side from `ExternalApp.organizationId` and `Membership`.
...
This preserves organization isolation: changing a `contractId` cannot grant access to another organization's contract, even when the same user has memberships in multiple organizations.
...
OPTION bindings must have the same active internal option values across datasets; matching labels alone are insufficient. RECORD bindings must target the same entity. Legacy unbound filters remain accepted but have no effect. A missing required filter returns `PANEL_FILTER_REQUIRED` before the affected dataset is queried. For a composed KPI, a missing required filter on the secondary operand makes that module unavailable with a selection-specific reason without failing independent modules.
DATE-bound `DATETIME` fields accept the ISO instant emitted by Client's date-time control; invalid date values do not become an empty-date comparison.
...
Datasets may include `relatedFields: [{ "relationFieldId": "<source relation field id>", "fieldId": "<target field id>" }]`. For each selected direct relation, Core returns the target field in `datasets[].schema.fields` and row `values` under `related:<relationFieldId>:<fieldId>`. That ID can be used by TABLE columns, PANEL_FILTER bindings, and metric conditions. The target field must be active, non-RELATION, and in an active entity of the same contract. Multiple direct relations are independent. A MANY relation returns an array in one source row; filters/conditions match any member and never expand row counts. Related values are fetched from the stored target record IDs before RECORDS/LATEST_BY_RELATION transformation, while metric conditions still run after transformation. Existing configs without `relatedFields` retain their behavior and cache revision changes when related selections change.
...
Assigned PANEL AppViews in `/views` also include `configRevision`, matching the PANEL response revision. Clients can use it to reject offline snapshots from a different saved configuration before the first PANEL request succeeds.
...
Metrics are calculated server-side for the executed dataset. When a KPI module anchored to that dataset has `visualization.config.composition = { metricAId, metricBId, operation }`, Core also executes metric B's dataset in the same request if different, using the same filter/search selection. The response still contains rows only for the requested dataset; `metrics` may additionally contain metric B, and `moduleResults` contains `{ moduleId, value, reason }` for each anchored composed KPI. A missing/non-numeric operand or zero divisor produces `value: null` and a reason; no independent KPI is zero-filled. Modules without `composition` continue using `metricId`. The execution order is: dataset static filters, required panel filters, optional panel filters selected by `metric.filterIds`, search, dataset transformation, `metric.conditions`, aggregation, composition, then pagination for visible rows only. For `LATEST_BY_RELATION`, metric conditions run after grouping. Response pagination only affects `datasets[].rows`, never metric or composed values. `PERCENT` with `percentScale = RATIO` formats a result of `0.75` as `75 %` in Client; Core returns `0.75`. The configuration never accepts SQL, JavaScript, expressions, or field names as logic; all references use ids.

`metric.conditions` is optional; absence is equivalent to an empty array. Conditions are combined with AND and affect only their metric. Operators are `EQUALS`, `NOT_EQUALS`, `IN`, `NOT_IN`, `IS_EMPTY`, and `IS_NOT_EMPTY`. SELECT and MULTISELECT conditions use stable `optionId` values, BOOLEAN uses booleans, TEXT/TEXTAREA use strings, INTEGER/DECIMAL/MONEY use numbers, and DATE/DATETIME use canonical date strings. `IS_EMPTY` matches null, absent values, empty strings, and empty arrays; `0` and `false` are not empty. RELATION conditions are not part of this stage until Web has a safe record selector.
