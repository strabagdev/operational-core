...
PANEL v1 stores a `schemaVersion = 1`, panel-level `filters`, `datasets`, reserved empty `metrics` and `calculatedFields` arrays, and visual `modules`. Only `source.type = ENTITY`, dataset transformations `RECORDS` and `LATEST_BY_RELATION`, and module visualization `TABLE` are executable in this stage. KPI, CHART, BOARD, GANTT, TEXT, metrics, calculated fields, JavaScript, SQL snippets, and arbitrary expressions are rejected as configuration errors instead of falling back silently.

Dataset execution happens in Core under the requesting user's AppView assignment and contract access. Every semantic reference is an `entityTypeId`, `fieldId`, `optionId`, or record id; names and field keys are presentation metadata only. `RECORDS` datasets apply configured filters and search before paginating. `LATEST_BY_RELATION` applies filters before reducing to one record per related record, orders by the configured `orderFieldId` descending, and uses only `record.id` as the tie breaker. The shared latest-by-relation reducer is reused by PANEL and legacy REPORT so the selection rule stays consistent without changing the public REPORT response.

TABLE modules render from `visualization.config.columns` exactly as configured. Core does not prepend `relationFieldId`, does not infer columns from dataset `fieldIds`, and rejects duplicate column field ids. A relation column can be included in any configured position as long as the field exists in the dataset schema.

Opco Web administration edits PANEL AppViews through a visual configuration form, not through raw JSON. The editor is ordered as data sources, filters, modules, and layout. Data sources still point to existing configurable `EntityType` definitions; PANEL does not create an entity, mutate records, or store transactional information. The form serializes the same `PanelConfig` contract used by the external API: dataset-level `source`, `filters`, `datasets`, `modules`, and reserved empty `metrics` and `calculatedFields`. Unknown properties are rejected until the published contract adds them explicitly; future compatibility must be introduced by versioning or extending the schema in one place, not by accepting arbitrary JSON.
