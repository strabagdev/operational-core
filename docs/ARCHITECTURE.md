...
TABLE modules render from `visualization.config.columns` exactly as configured. Core does not prepend `relationFieldId`, does not infer columns from dataset `fieldIds`, and rejects duplicate column field ids. A relation column can be included in any configured position as long as the field exists in the dataset schema. KPI modules reference `visualization.config.metricId`; the metric definition owns the dataset, aggregation, optional `fieldId`, optional `filterIds`, optional `conditions`, and the module owns only presentation config. `MONEY` KPI presentation requires an explicit ISO 4217 `currencyCode`; `PERCENT` requires an explicit `percentScale`, where `RATIO` renders `0.25` as `25 %` and `WHOLE` renders `25` as `25 %`. Those properties are rejected on incompatible formats.
