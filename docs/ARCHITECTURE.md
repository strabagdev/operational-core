...
- `CURRENT_STATUS`: records are reduced to one current row per related record. `currentStatus.subjectFieldId` optionally points to the relation field that identifies the related record, `currentStatus.stateFieldId` is required, and `currentStatus.dateFieldId` is optional. When the date field is configured, multiple historical rows for the same related record are reduced to the latest by that field and ordered descending. When it is absent, Opco does not use `createdAt`, `updatedAt`, or audit metadata as a substitute.
